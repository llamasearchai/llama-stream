...
class StreamEvent(Generic[T]):
    ...
    def __str__(self) -> str:
        ...
        return f"StreamEvent(id={self.id}, source={self.source_id}, ts={self.timestamp})"
...
class EventSource(ABC, Generic[T]):
    ...
    def emit(self, data: T, metadata: Optional[Dict[str, Any]] = None) -> StreamEvent[T]:
    ...
    def to_mlx_array(self) -> "mx.array":
        ...
        for event in self.events:
            if isinstance(event.data, dict):
                ...
                feature_vector = [
                    float(v) for k, v in event.data.items() if isinstance(v, (int, float))
                ]
...
class WindowManager:
    ...
    def _assign_to_tumbling_window(self, event: StreamEvent, spec: WindowSpec) -> Window:
    ...
    def _assign_to_sliding_windows(self, event: StreamEvent, spec: WindowSpec) -> List[Window]:
    ...
    def _assign_to_session_window(self, event: StreamEvent, spec: WindowSpec) -> Window:
        ...
        if session_key in self.active_sessions:
            ...
            last_event_time = (
                max(e.timestamp for e in session.events) if session.events else session.start_time
            )
            ...
            new_session = Window(
                window_id=f"session_{session_key}_{event.timestamp}",
                start_time=event.timestamp,
                metadata={"spec_key": spec, "window_type": "session", "session_key": session_key},
            )
            self.active_sessions[session_key] = new_session
            self.windows[new_session.window_id] = new_session
            new_session.add_event(event)
    ...
    def _assign_to_global_window(self, event: StreamEvent) -> Window:
        ...
        if window_id not in self.windows:
            window = Window(window_id=window_id, start_time=0, metadata={"window_type": "global"})
            self.windows[window_id] = window
        else:
            window = self.windows[window_id]
...
class StreamProcessor:
    ...
    def add_operator(self, operator: Callable, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        while not self._shutdown_flag.is_set():
            try:
                ...
                for operator, config in self.operators:
                    ...
                    if "source_filter" in config and event.source_id not in config["source_filter"]:
                        continue
                    ...
                    if "window_spec" in config:
                        ...
                        for window in windows:
                            if window.is_closed() or (
                                config.get("process_incomplete", False) and len(window) > 0
                            ):
                                try:
                                    ...
                                    logger.error(f"Error in operator {operator.__name__}: {str(e)}")
                    else:
    ...
    def _publish_result(self, result: Any, window: Optional[Window], config: Dict) -> None:
        ...
        source_id = window.events[0].source_id if window and window.events else "processor"
        event = StreamEvent(
            data=result,
            source_id=source_id,
            metadata={
    ...
    def _checkpoint_state(self) -> None:
        ...
        try:
            ...
            checkpoint = {"state": self.state, "timestamp": time.time(), "version": "1.0"}
...
class KafkaSource(EventSource[Dict]):
    ...
    def __init__(self, source_id: str, bootstrap_servers: List[str], topic: str, **kafka_config):
